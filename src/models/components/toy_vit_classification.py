...
class ToyViTClassification(torch.nn.Module):
    def __init__(
        ...
        self.input_size = self.img_size * 2 + self.img_height
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, 1 + self.input_size, self.embed_dim)
        )  # For CLS
    ...
    def forward(self, x):
        ...
        # print(f"All xs are equal?: {torch.all(x[0,:,:] == x[1,:,:])}")
        predictions = self.fc(x[:, 0, :]).squeeze(-1)
        # print(f"All predictions are equal?: {torch.all(predictions == predictions[0])}")

        # print(f"An x: {x[0, :]}")
        # print(f"Another x: {x[1, :]}")
        # print(f"A prediction: {predictions[0]}")
